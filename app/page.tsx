...
// ═══════════════════════════════════════════════════════════════════
// BRIGHT ANIMATED GRID — visible electric-blue grid behind ER input
// ═══════════════════════════════════════════════════════════════════
function AnimatedGrid() {
  return (
    <>
      {/* Outer container — bleeds 3px outside the card so the glow wraps it */}
      <div aria-hidden style={{
        position: "absolute", inset: -3, borderRadius: 16,
        overflow: "hidden", pointerEvents: "none", zIndex: 0,
      }}>
        {/* ── Fine grid (every 32px) — bright cyan lines ── */}
        <div style={{
          position: "absolute", inset: 0,
          backgroundImage: `
            linear-gradient(rgba(0, 200, 255, 0.55) 1.5px, transparent 1.5px),
            linear-gradient(90deg, rgba(0, 200, 255, 0.55) 1.5px, transparent 1.5px)
          `,
          backgroundSize: "32px 32px",
          animation: "gridBreathe 3s ease-in-out infinite",
        }} />

        {/* ── Major grid (every 96px) — extra-bright accent lines ── */}
        <div style={{
          position: "absolute", inset: 0,
          backgroundImage: `
            linear-gradient(rgba(0, 230, 255, 0.85) 2px, transparent 2px),
            linear-gradient(90deg, rgba(0, 230, 255, 0.85) 2px, transparent 2px)
          `,
        ...
        <div style={{
          position: "absolute", inset: 0,
          background: "linear-gradient(180deg, transparent 0%, rgba(0,200,255,0.10) 50%, transparent 100%)",
        ...
        <div style={{
          ...
          boxShadow: "0 0 28px rgba(0,210,255,0.7), inset 0 0 28px rgba(0,210,255,0.18)",
        ...
        {([
          ...
          <div key={i} style={{
            ...
            boxShadow: "0 0 10px rgba(0,230,255,0.9)",
        ...
        <div style={{
          ...
          boxShadow: "0 0 14px 5px rgba(0,200,255,0.6)",
          animation: "dotPulse 2.2s ease-in-out infinite",
        }} />
      </div>

      {/* Global keyframes for all grid animations */}
      <style>{`
        ...
        @keyframes borderPulse {
          0%,100% { box-shadow: 0 0 18px rgba(0,210,255,0.55), inset 0 0 18px rgba(0,210,255,0.12); }
          50%      { box-shadow: 0 0 38px rgba(0,230,255,0.95), inset 0 0 32px rgba(0,230,255,0.25); }
        }
        @keyframes cornerPulse {
          0%,100% { opacity: 0.7; }
          50%      { opacity: 1; box-shadow: 0 0 16px rgba(0,230,255,1); }
        }
        @keyframes dotPulse {
        ...
        mermaid.initialize({
          ...
          themeVariables: darkMode
            ? { primaryColor:"#1e40af", primaryTextColor:"#e2e8f0",
                ...
                background:"#0a0a0a", mainBkg:"#111827", nodeBorder:"#3b82f6",
                clusterBkg:"#111827", titleColor:"#60a5fa",
                edgeLabelBackground:"#1f2937", tertiaryColor:"#1f2937" }
    ...
    <span className="relative inline-block">
      <span onMouseEnter={()=>setShow(true)} onMouseLeave={()=>setShow(false)}
        className={`cursor-help border-b-2 border-dotted transition-colors ${
          darkMode ? "border-blue-400 text-blue-300 hover:text-blue-200"
                   : "border-indigo-400 text-indigo-700 hover:text-indigo-900"
        }`}>{term}</span>
      {show && (
        <span className={`absolute bottom-full mb-2 z-50 w-60 rounded-lg p-3 text-xs shadow-2xl pointer-events-none ${
          darkMode ? "bg-blue-900/95 border border-blue-500/50 text-blue-100"
          ...
          {translation}
          <span className={`absolute top-full border-4 border-transparent ${darkMode ? "border-t-blue-900/95" : "border-t-white"}`}
...
function GlossaryText({ text, glossary, darkMode }: { text:string; glossary:GlossaryItem[]; darkMode:boolean }) {
  ...
  return <>{parts.map((p,i) =>
    map[p.toLowerCase()]
      ? <GlossaryTerm key={i} term={p} translation={map[p.toLowerCase()]} darkMode={darkMode}/>
      : <span key={i}>{p}</span>
  )}</>;
}
  ...
  return (
    <button onClick={()=>{ navigator.clipboard.writeText(text); setCopied(true); setTimeout(()=>setCopied(false),2000); }}
      className={`flex items-center gap-1 text-xs px-2 py-1 rounded transition-all ${
        darkMode ? "bg-blue-900/40 hover:bg-blue-800/60 text-blue-300 border border-blue-700/50"
...
function WeaveButton({ loading, disabled, onClick, dark=false }: {
  loading:boolean; disabled:boolean; onClick:()=>void; dark?:boolean;
}) {
  return (
    <button onClick={onClick} disabled={disabled}
      className={`relative flex items-center justify-center gap-2 px-6 py-2.5 font-semibold text-sm text-white
        transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed overflow-hidden select-none
        ${dark ? "rounded-sm" : "rounded-xl"}`}
      style={loading
        ? { background:"linear-gradient(90deg,#0369a1,#0ea5e9,#0369a1)", backgroundSize:"200%",
            animation:"gradMove 1.4s linear infinite" }
        : dark
          ? { background:"#0ea5e9", boxShadow:"0 0 24px rgba(14,165,233,0.6), 0 0 48px rgba(14,165,233,0.2)" }
          : { background:"#1e293b", boxShadow:"0 4px 20px rgba(79,70,229,.3)" }
      }
    >
      {loading && (
      ...
      <span className="relative flex items-center gap-2">
        {loading
          ? <><Loader2 size={14} className="animate-spin"/>{dark ? "WEAVING…" : "Weaving…"}</>
          : <><Sparkles size={14}/>{dark ? "✦ WEAVE SPEC" : "Weave Architecture"}</>
        }
      </span>
    </button>
  );
    ...
    <div style={{animation:"fadeInUp .45s ease both"}} className="space-y-5 sm:space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 sm:p-7 shadow-sm">
        ...
        <p className="text-slate-600 leading-relaxed text-sm">
          <GlossaryText text={overview} glossary={glossary} darkMode={false}/>
        </p>
      </div>
      ...
      <div className="bg-slate-900 rounded-2xl p-5 sm:p-8 text-white">
        ...
        <p className="text-slate-300 leading-relaxed text-sm">
          <GlossaryText text={executiveInsights} glossary={glossary} darkMode={true}/>
        </p>
      </div>
            ...
            <div key={i} className="bg-black/50 rounded-lg p-3 sm:p-4" style={{border:"1px solid rgba(0,140,255,0.2)"}}>
              <div className="flex flex-wrap items-center gap-2 mb-3">
                <span className={`text-xs font-bold tracking-wider font-mono ${
                  ...
                  : ep.method==="PUT" || ep.method==="PATCH" ? "text-amber-400"
                  : "text-red-400"
                }`}>{ep.method}</span>
...
export default function Specweaver() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setBoardroomData(null); setEngineData(null);
    try {
      const res = await fetch("/api/weaver", {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ prompt, viewMode }),
      });
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/30"
      ...
      <header className="border-b border-slate-200/80 bg-white/70 backdrop-blur-xl sticky top-0 z-40">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-3">
          ...
          <button onClick={()=>setViewMode("EngineRoom")}
            className="flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-full border border-slate-200 bg-white hover:bg-slate-50 transition-all text-xs sm:text-sm text-slate-600 shadow-sm">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 sm:p-6 mb-6 sm:mb-8">
          ...
          <textarea id="br-prompt" value={prompt} onChange={handlePromptChange} onKeyDown={handleKeyDown}
            placeholder="e.g. A healthcare platform where patients can book appointments, doctors can manage schedules, and administrators view analytics across 50+ clinics…"
            rows={4}
            className="w-full text-sm text-slate-800 placeholder-slate-400 bg-slate-50 border border-slate-200 rounded-xl p-4 resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 transition-all"/>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mt-4">
            <p className="text-xs text-slate-400">Board-ready output, no jargon · <kbd className="bg-slate-100 border border-slate-200 rounded px-1 font-mono">⌘ Enter</kbd></p>
        ...
        {loading && (
          <div className="space-y-4 animate-pulse">
            <div className="grid grid-cols-3 gap-4">{[0,1,2].map(i=><div key={i} className="h-24 bg-slate-100 rounded-2xl"/>)}</div>
            <div className="h-32 bg-slate-100 rounded-2xl"/>
            <div className="h-48 bg-slate-100 rounded-2xl"/>
          </div>
        )}
        {boardroomData && !loading && <BoardroomOutput data={boardroomData}/>}
        {!boardroomData && !loading && (
          <div className="text-center py-16 sm:py-24 text-slate-400">
            <Layers size={44} className="mx-auto mb-4 opacity-20"/>
            <p className="text-sm">Enter your product vision above to generate your architecture overview.</p>
          </div>
        )}
      </main>
    ...
    <div className="min-h-screen text-slate-300"
      ...
      <header className="border-b border-cyan-900/50 bg-black/80 backdrop-blur-xl sticky top-0 z-40"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
          ...
          <button onClick={()=>setViewMode("Boardroom")}
            className="flex items-center gap-1.5 px-3 py-2 border border-cyan-800/60 rounded text-xs text-cyan-400 hover:border-cyan-500 hover:bg-cyan-950/30 transition-all font-mono">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10 relative z-10">
        <div className="mb-7 sm:mb-10" style={{animation:"slideDown .5s ease both"}}>
          ...
          <h2 className="text-3xl sm:text-4xl font-bold text-white mb-1.5"
            style={{textShadow:"0 0 40px rgba(0,200,255,0.6), 0 0 80px rgba(0,200,255,0.2)"}}>
            The Engine Room
          </h2>
          <p className="text-xs sm:text-sm text-cyan-500/70 font-mono">Production-grade technical specifications. Zero ambiguity.</p>
        </div>

        {/* ── INPUT BOX with AnimatedGrid ──────────────────────────── */}
        <div className="relative mb-6 sm:mb-8" style={{padding: 4}}>
          <AnimatedGrid />
          <div className="relative z-10 rounded-xl p-4 sm:p-6"
            style={{background:"rgba(5,12,24,0.95)", backdropFilter:"blur(6px)"}}>
            <div className="flex items-center gap-2 mb-3">
              <Terminal size={13} className="text-cyan-400"/>
              <span className="text-xs text-cyan-400 tracking-widest font-mono">SYSTEM_PROMPT.INPUT</span>
            </div>
            <textarea
              id="er-prompt" value={prompt}
              onChange={handlePromptChange} onKeyDown={handleKeyDown}
              placeholder={"// Describe the system to architect...\n// e.g. Real-time collaborative doc editor with conflict resolution, offline support, multi-tenant arch..."}
              rows={5}
              ...
              onFocus={e=>{e.currentTarget.style.borderColor="rgba(0,220,255,0.7)"; e.currentTarget.style.boxShadow="inset 0 0 20px rgba(0,0,0,0.6), 0 0 12px rgba(0,200,255,0.2)";}}
              onBlur={e=>{e.currentTarget.style.borderColor="rgba(0,180,255,0.35)"; e.currentTarget.style.boxShadow="inset 0 0 20px rgba(0,0,0,0.6)";}}
            />
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mt-4">
        ...
        {error && (
          <div className="rounded-xl p-4 mb-5 flex items-start gap-3"
            ...
            <p className="text-xs text-red-300 font-mono break-words">[ERROR] {error}</p>
          </div>
        )}

        {loading && (
          <div className="space-y-4">
            {[120, 200, 140, 100].map((h,i)=>(
              <div key={i} className="rounded-xl" style={{
                height:h, border:"1px solid rgba(0,160,255,0.15)",
                background:"rgba(8,18,36,0.8)",
                animation:`skeletonPulse 1.6s ${i*0.15}s ease-in-out infinite`,
              }}/>
            ))}
          </div>
        )}
        ...
        {!engineData && !loading && (
          <div className="text-center py-16 sm:py-24">
            <Cpu size={44} className="mx-auto mb-4" style={{color:"rgba(0,160,255,0.25)", filter:"drop-shadow(0 0 16px rgba(0,200,255,0.3))"}}/>
            <p className="text-xs text-cyan-800 font-mono">// AWAITING INPUT — SYSTEM READY</p>
          </div>
        )}
      </main>
